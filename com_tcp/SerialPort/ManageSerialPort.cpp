...
void ThreadSend::run()
{
	...
	forever
	{
        ...
        //qDebug("dequeue:%x", byteArray.at(0));
	}
}
...
void ThreadReceive::run()
{
	...
	forever
	{
		...
		if (numBytes > 0)
		{
            ...
            //dataReceived = QByteArray(data);//不发送0x0
            //包括0x0
            dataReceived = QByteArray::fromRawData(data, numBytes);
            //qDebug("size1 is %d d is %d", numBytes, dataReceived.size());
			mutexReceive.unlock();
//            for(int i = 0; i < numBytes; i++) {
//                qDebug("receive2:%x", data[i]);
//                //qDebug("receive:%x", dataReceived.at(i));
//            }
//            qDebug("data size3 is %d", dataReceived.size());
